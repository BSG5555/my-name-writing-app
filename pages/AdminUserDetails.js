...
export default function AdminUserDetails() {
    ...
    const handleUnlockUpload = async () => {
        ...
        try {
            ...
            if (missedDays.length > 0) {
                const confirm = window.confirm(`Unlock ${missedDays.length} missed days for ${user.full_name}?`);
                if (!confirm) {
    ...
    const stats = {
        ...
        totalPaid: payments.filter(p => p.status === 'APPROVED').reduce((sum, p) => sum + p.amount, 0),
    };
                ...
                <CardContent>
                    <Calendar
                        ...
                        modifiers={{
                            ...
                            paid: payments.filter(p => p.status === 'APPROVED').map(p => new Date(p.date)),
                        }}
                        modifiersClassNames={{
